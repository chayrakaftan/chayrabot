...
export function getSuggestions(lang: Lang): string[] {
  if (lang === 'en') return ['Where is my order?', 'Shipping', 'Delivered before Eid?', 'Size guide', 'Fabrication', 'Contact us']
  if (lang === 'ar') return ['أين طلبي؟', 'التوصيل', 'التوصيل قبل العيد؟', 'دليل المقاسات', 'الصناعة', 'اتصل بنا']
